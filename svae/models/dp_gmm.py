...
def init_pgm_param(T, N, alpha, niw_conc=10., random_scale=0.):
    ...
    dp_natparam = np.stack([np.array([1, alpha]) for _ in range(T-1)])
    niw_natparam = np.stack([init_niw_natparam(N) for _ in range(T)])

    return dp_natparam, niw_natparam

def prior_logZ(gmm_natparam):
    dp_natparam, niw_natparams = gmm_natparam
    return dp.logZ(dp_natparam) + niw.logZ(niw_natparams)

def prior_expectedstats(gmm_natparam):
    dp_natparam, niw_natparams = gmm_natparam
    dp_expectedstats = dp.expectedstats(dp_natparam)
    niw_expectedstats = niw.expectedstats(niw_natparams)
    return dp_expectedstats, niw_expectedstats
...
def label_meanfield(label_global, gaussian_globals, gaussian_stats):
    # Ref. Eq 39 in [1]
    # For Dirichlet label_global = E_{q(\pi)}[t(\pi)] where q(\pi) is dirichlet and t(\pi) is {log\pi_i}
    # But in a DP, it is given by E_{q(\pi)}[log p(Z|V)] (Refer [2] for form)
